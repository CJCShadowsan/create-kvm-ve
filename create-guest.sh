...
function usage {
  ...
  echo "  PRIMARY SUBNET: subnet for eth0, in CIDR notation, e.g. 10.70.73.0/16"
  echo "  PRIMARY GATEWAY: gateway (IPv4 address) for eth0"
...
awk '{ \
  ...
  printf "network --hostname=%s --onboot=yes --device=eth0 --noipv6 --bootproto=%s --ip=%s --netmask=%s --gateway=%s\n",hn,bp,ip,mk,gw; \
  if (length(sip)>0) {printf "network --onboot=yes --device=eth1 --noipv6 --gateway=0.0.0.0 --bootproto=%s --ip=%s --netmask=%s\n",sbp,sip,smk} \
...
if [ "$SECONDARY_NET" == "" ]; then
  virt-install --name $VMNM \
    ...
    --extra-args "ks=file:/$USER-$VMNM.ks console=tty0 console=ttyS0,115200 vehostname=$VMNM ve1st_net=$PRIMARY_NET ve1st_netmask=$PRIMARY_SUBNET_MASK ve1st_gw=$PRIMARY_GW veprefix=$VEPREFIX" \
  ...
  virt-install --name $VMNM \
    ...
    --graphics none \
    --initrd-inject="/tmp/$USER-$VMNM.ks" \
    --extra-args "ks=file:/$USER-$VMNM.ks console=tty0 console=ttyS0,115200 vehostname=$VMNM ve1st_net=$PRIMARY_NET ve1st_netmask=$PRIMARY_SUBNET_MASK ve1st_gw=$PRIMARY_GW ve2nd_net=$SECONDARY_NET ve2nd_netmask=$SECONDARY_SUBNET_MASK veprefix=$VEPREFIX" \
    --disk "$VMROOT/$VMNM,device=disk,bus=virtio,size=10,sparse=true,format=raw" \
    --network bridge=$PRIBRIDGE,mac=$PRI_MAC \
